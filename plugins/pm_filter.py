...
from database.ia_filterdb import Media, get_file_details, get_search_results, get_bad_files, get_available_languages
...
async def get_language_buttons(query, userid):
    langs_map = {
        'mal': 'Mᴀʟᴀʏᴀʟᴀᴍ',
        'tam': 'Tᴀᴍɪʟ',
        'hin': 'Hɪɴᴅɪ',
    ...
    # Strip existing language tags to find the base movie
    base_query = query
    for lang_code in langs_map.keys():
        if base_query.endswith(f" {lang_code}"):
            base_query = base_query.rsplit(' ', 1)[0]
            break
            
    available_lang_codes = await get_available_languages(base_query)
    
    if len(available_lang_codes) > 1:
        available_langs = []
        for code in available_lang_codes:
            name = langs_map.get(code, code.capitalize())
            available_langs.append(InlineKeyboardButton(name, callback_data=f"lang#{userid}#{code}"))
        
        # Sort for consistency
        available_langs.sort(key=lambda x: x.text)
        
        # Return buttons in rows of 3
        return [available_langs[i:i + 3] for i in range(0, len(available_langs), 3)]
    return None


@Client.on_message(filters.group & filters.text & filters.incoming)
async def give_filter(client: Client, message: pyrogram.types.Message):
...
async def pm_text(bot: Client, message: pyrogram.types.Message):
    ...
    if user_id in ADMINS:
        if user_id in temp.FSUB_STATE:
            state = temp.FSUB_STATE.pop(user_id)
            try:
                channel_id = int(content)
            except:
                return await message.reply_text("<b>Invalid Channel ID. Please send a numeric ID starting with -100.</b>")
            
            config_key = 'AUTH_CHANNEL' if state == 1 else 'AUTH_CHANNEL2'
            await db.update_config(config_key, channel_id)
            import info
            if state == 1: info.AUTH_CHANNEL = channel_id
            else: info.AUTH_CHANNEL2 = channel_id
            
            buttons = [[InlineKeyboardButton('⟸ Bᴀᴄᴋ Tᴏ F Sᴜʙ', callback_data='fsub_settings')]]
            return await message.reply_text(
                f"<b>Sᴜᴄᴄᴇssғᴜʟʟʏ sᴇᴛ {config_key} ᴛᴏ <code>{channel_id}</code>.</b>",
                reply_markup=InlineKeyboardMarkup(buttons)
            )
        return # ignore other admin PMs
...
async def next_page(bot: Client, query: CallbackQuery):
    ...
    # Removed dupe/reqinfo/sinfo buttons as requested
    lang_btns = await get_language_buttons(search, query.from_user.id)
    if lang_btns:
        for row in reversed(lang_btns):
            btn.insert(0, row)
    try:
        if settings['max_btn']:
            if 0 < offset <= 10:
...
async def language_check(bot, query):
    ...
    if files:
        ...
        lang_btns = await get_language_buttons(movie, userid)
        if lang_btns:
            for row in reversed(lang_btns):
                btn.insert(0, row)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if query.from_user.id in ADMINS:
            buttons.append([InlineKeyboardButton('Aᴅᴍɪɴ Pᴀɴᴇʟ', callback_data='admin_panel')])
        ...
        buttons = [[
            InlineKeyboardButton('F Sᴜʙ Sᴇᴛᴛɪɴɢs', callback_data='fsub_settings')
        ],[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    elif query.data == "fsub_settings":
        import info
        buttons = [[
            InlineKeyboardButton('Sᴇᴛ F Sᴜʙ 1', callback_data='set_fsub1'),
            InlineKeyboardButton('Sᴇᴛ F Sᴜʙ 2', callback_data='set_fsub2')
        ],[
            InlineKeyboardButton('Dɪsᴀʙʟᴇ 1', callback_data='disable_fsub1'),
            InlineKeyboardButton('Dɪsᴀʙʟᴇ 2', callback_data='disable_fsub2')
        ],[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='admin_panel')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.FSUB_TXT.format(info.AUTH_CHANNEL, info.AUTH_CHANNEL2),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
    elif query.data == "set_fsub1":
        temp.FSUB_STATE[query.from_user.id] = 1
        await query.answer("Sᴇɴᴅ ᴛʜᴇ Nᴇᴡ Cʜᴀɴɴᴇʟ ID ɴᴏᴡ...", show_alert=True)
        await query.message.edit_text(
            "<b>Pʟᴇᴀsᴇ sᴇɴᴅ ᴛʜᴇ ɴᴇᴡ Cʜᴀɴɴᴇʟ ID ғᴏʀ Fᴏʀᴄᴇ Sᴜʙsᴄʀɪʙᴇ 1.\n\nExᴀᴍᴘʟᴇ: <code>-100123456789</code></b>",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Bᴀᴄᴋ', callback_data='fsub_settings')]])
        )
    elif query.data == "set_fsub2":
        temp.FSUB_STATE[query.from_user.id] = 2
        await query.answer("Sᴇɴᴅ ᴛʜᴇ Nᴇᴡ Cʜᴀɴɴᴇʟ ID ɴᴏᴡ...", show_alert=True)
        await query.message.edit_text(
            "<b>Pʟᴇᴀsᴇ sᴇɴᴅ ᴛʜᴇ ɴᴇᴡ Cʜᴀɴɴᴇʟ ID ғᴏʀ Fᴏʀᴄᴇ Sᴜʙsᴄʀɪʙᴇ 2.\n\nExᴀᴍᴘʟᴇ: <code>-100123456789</code></b>",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Bᴀᴄᴋ', callback_data='fsub_settings')]])
        )
    elif query.data == "disable_fsub1":
        import info
        await db.update_config('AUTH_CHANNEL', None)
        info.AUTH_CHANNEL = None
        await query.answer("Force Subscribe 1 Disabled", show_alert=True)
        # Refresh the fsub settings page
        buttons = [[
            InlineKeyboardButton('Sᴇᴛ F Sᴜʙ 1', callback_data='set_fsub1'),
            InlineKeyboardButton('Sᴇᴛ F Sᴜʙ 2', callback_data='set_fsub2')
        ],[
    ...
    elif query.data == "disable_fsub2":
        import info
        await db.update_config('AUTH_CHANNEL2', None)
        info.AUTH_CHANNEL2 = None
        await query.answer("Force Subscribe 2 Disabled", show_alert=True)
        # Refresh the fsub settings page
        buttons = [[
            InlineKeyboardButton('Sᴇᴛ F Sᴜʙ 1', callback_data='set_fsub1'),
            InlineKeyboardButton('Sᴇᴛ F Sᴜʙ 2', callback_data='set_fsub2')
...
async def auto_filter(client, msg, spoll=False):
    ...
    # Removed dupe/reqinfo/sinfo buttons as requested
    lang_btns = await get_language_buttons(search, message.from_user.id)
    if lang_btns:
        for row in reversed(lang_btns):
            btn.insert(0, row)
