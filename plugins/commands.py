...
async def start(client: Client, message: pyrogram.types.Message):
    ...
    if len(message.command) != 2:
        buttons = [
            ...
            [
                InlineKeyboardButton('⚜️ Join Movie Group ⚜️', url=GRP_LNK)
            ]
        ]
        if message.from_user.id in ADMINS:
            buttons.append([InlineKeyboardButton('Aᴅᴍɪɴ Pᴀɴᴇʟ', callback_data='admin_panel')])
    ...
    if not files_:
        try:
            # Fallback for base64 encoded file IDs
        ...
        files_ = await get_file_details(file_id)
        if not files_:
            # If still not found, it might be a direct Telegram File ID
            # In some bots, they send the actual file ID in base64
            pass

    if not files_:
        # If it's not in our database but looks like a Telegram file ID, we could still try to send it
        # But for this bot, we usually want it to be in our database for logging/stats.
        # Let's try sending it directly if it looks like a valid file_id
        if len(file_id) > 20: 
            try:
                await client.send_cached_media(
                    chat_id=message.from_user.id,
                    file_id=file_id,
                    protect_content=True if kk == 'filep' else False,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url=CHNL_LNK)]])
                )
                return
            except:
                pass
...
async def delete(bot: Client, message: pyrogram.types.Message):
    ...
    if result.deleted_count:
        ...
        if result.deleted_count:
            ...
            # legacy indexed files compatibility
...
async def set_fsub(client, message):
    if len(message.command) < 2:
        return await message.reply("<b>Please provide a Channel ID.\nExample: <code>/set_fsub -100123456789</code></b>")
    channel_id = message.command[1]
    if channel_id.lower() == "none":
        await db.update_config('AUTH_CHANNEL', None)
        import info
        info.AUTH_CHANNEL = None
        return await message.reply("<b>Successfully disabled AUTH_CHANNEL.</b>")
    try:
        channel_id = int(channel_id)
    except:
        return await message.reply("<b>Invalid Channel ID.</b>")
    await db.update_config('AUTH_CHANNEL', channel_id)
    import info
    info.AUTH_CHANNEL = channel_id
    await message.reply(f"<b>Successfully set AUTH_CHANNEL to <code>{channel_id}</code>.</b>")

@Client.on_message(filters.command("set_fsub2") & filters.user(ADMINS))
async def set_fsub2(client, message):
    if len(message.command) < 2:
        return await message.reply("<b>Please provide a Channel ID.\nExample: <code>/set_fsub2 -100123456789</code></b>")
    channel_id = message.command[1]
    if channel_id.lower() == "none":
        await db.update_config('AUTH_CHANNEL2', None)
        import info
        info.AUTH_CHANNEL2 = None
        return await message.reply("<b>Successfully disabled AUTH_CHANNEL2.</b>")
    try:
        channel_id = int(channel_id)
    except:
        return await message.reply("<b>Invalid Channel ID.</b>")
    await db.update_config('AUTH_CHANNEL2', channel_id)
    import info
    info.AUTH_CHANNEL2 = channel_id
    await message.reply(f"<b>Successfully set AUTH_CHANNEL2 to <code>{channel_id}</code>.</b>")

@Client.on_message(filters.command("fsub") & filters.user(ADMINS))
async def fsub_info_cmd(client, message):
    import info
    text = f"<b>Force Subscribe Settings:</b>\n\n"
    text += f"• <b>AUTH_CHANNEL:</b> <code>{info.AUTH_CHANNEL}</code>\n"
    text += f"• <b>AUTH_CHANNEL2:</b> <code>{info.AUTH_CHANNEL2}</code>\n"
    await message.reply(text)
